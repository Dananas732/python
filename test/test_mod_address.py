...
def test_modify_address_name(app):
    ...
    address.id = old_address_list[index].id     #присваиваем созданной переменной ID первого по списку ID адреса (так как при изменении адреса выбирается первый по списку адрес)
    app.address.modify(index, address)     #процедура модификации адреса
    ...
    old_address_list[index] = address       #присваиваем параметры модификации первой записи в списке групп. ID уже было присвоено ранее
